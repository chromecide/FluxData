	...
	function objectConstructor(EventEmitter2, util){
		...
		function channel(cfg, callback){
			...
			channel.prototype.addInput = function(name, accepts){
				this.inputs.push({
					name: name,
					accepts: accepts
				});
			...
			channel.prototype.addOutput = function(name, emits){
				this.outputs.push({
					name: name,
					emits: emits
				});
			...
			channel.prototype.remove = function(prop, index){
				var self = this;
				var oldValue = self.get(prop);
				...
				if(index && index>=0){
					...
					self._data = removeDataValueByString(self._data, prop);

					self.emit('value_removed.'+prop, {
						old: oldValue,
						new: currentValue,
						property: prop
					});
							...
							for(var key in mixin){
								switch(key){
									...
									case 'publish':
										...
										break;
									case 'inputs':
										for(var i=0;i<mixin.inputs.length;i++){
											self.addInput(mixin.inputs[i].name, mixin.inputs[i].emits);
										}
										break;
									case 'outputs':
										for(var o=0;o<mixin.outputs.length;o++){
											self.addOutput(mixin.outputs[o].name, mixin.outputs[o].emits);
										}
										break;
									default:
										self[key] = mixin[key];
			 ...
			 */
			channel.prototype.setDataValueByString = setDataValueByString = function(obj, prop, value){
				if(!obj){
					obj = {};
				}
			 ...
			 */
			channel.prototype.getDataValueByString = getDataValueByString = function (obj, prop){
			...
			channel.prototype.removeDataValueByString = removeDataValueByString = function (obj, prop){
				if(!obj){
					obj = {};
				}
				...
				if(prop.indexOf('.')>-1){
					var parts = prop.split('.');
					prop = parts.shift();
					obj[prop] = removeDataValueByString(obj[prop], parts.join('.'));
				}else{
					delete obj[prop];
				}
